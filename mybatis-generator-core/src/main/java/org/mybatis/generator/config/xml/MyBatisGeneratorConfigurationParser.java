...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseTable(Context context, Node node) {
        ...
        String enabledGxqptExt = attributes.getProperty("enabledGxqptExt"); //$NON-NLS-1$
        String enableAppId = attributes.getProperty("enableAppId"); //$NON-NLS-1$
        ...
        if (stringHasValue(enabledGxqptExt)) {
            tc.setGxqptExtStatementEnabled(isTrue(enabledGxqptExt));
        }
        if (stringHasValue(enableAppId)) {
            tc.setGxqptAppIdEnabled(isTrue(enableAppId));
        }
    ...
    private void parseJavaModelGenerator(Context context, Node node) {
        ...
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$
        String targetSuperClass = attributes.getProperty("targetSuperClass"); //tangyh
        String targetSuperGeneric = attributes.getProperty("targetSuperGeneric"); //tangyh
        String targetExamplePackage = attributes.getProperty("targetExamplePackage"); //tangyh
        String targetExampleProject = attributes.getProperty("targetExampleProject"); //tangyh
        String targetExampleSuperClass = attributes.getProperty("targetExampleSuperClass"); //tangyh
        String targetGeneratedCriteriaSuperClass = attributes.getProperty("targetGeneratedCriteriaSuperClass"); //tangyh
        if (targetExamplePackage == null || "".equals(targetExamplePackage)) {
            targetExamplePackage = targetPackage;
        }
        if (targetExampleProject == null || "".equals(targetExampleProject)) {
            targetExampleProject = targetProject;
        }

        javaModelGeneratorConfiguration.setTargetPackage(targetPackage);
        javaModelGeneratorConfiguration.setTargetProject(targetProject);
        javaModelGeneratorConfiguration.setTargetSuperClass(targetSuperClass);
        javaModelGeneratorConfiguration.setTargetSuperGeneric(targetSuperGeneric);
        javaModelGeneratorConfiguration.setTargetExamplePackage(targetExamplePackage);
        javaModelGeneratorConfiguration.setTargetExampleProject(targetExampleProject);
        javaModelGeneratorConfiguration.setTargetExampleSuperClass(targetExampleSuperClass);
        javaModelGeneratorConfiguration.setTargetGeneratedCriteriaSuperClass(targetGeneratedCriteriaSuperClass);
    ...
    private void parseJavaClientGenerator(Context context, Node node) {
        ...
        String targetSuperClass = attributes.getProperty("targetSuperClass"); //tangyh
        String targetSuperGeneric = attributes.getProperty("targetSuperGeneric"); //tangyh
        String targetIsGenMethod = attributes.getProperty("targetIsGenMethod"); //tangyh
        String implementationPackage = attributes.getProperty("implementationPackage"); //$NON-NLS-1$

        javaClientGeneratorConfiguration.setConfigurationType(type);
        javaClientGeneratorConfiguration.setTargetPackage(targetPackage);
        javaClientGeneratorConfiguration.setTargetProject(targetProject);
        javaClientGeneratorConfiguration.setTargetSuperClass(targetSuperClass);
        javaClientGeneratorConfiguration.setTargetIsGenMethod(targetIsGenMethod);
        javaClientGeneratorConfiguration.setTargetSuperGeneric(targetSuperGeneric);
